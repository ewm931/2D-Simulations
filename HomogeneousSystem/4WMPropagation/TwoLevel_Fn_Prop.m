function [] = TwoLevel_Fn_Prop(Index,wells,L,EIS,EID,gamma,E,probe)
global Folder
...
TStep = 2E-13;
TMin = -2E-12;
TAxis = TMin:TStep:(Size_T-1)*TStep+TMin;
...
tau = 0;%2E-13;          % Delay tau

% Full time axis parameters
TimeStart = TMin-7E-13;
...
% Define # well for propagation
%wells = 3; % Don't forget to change  mu
mu = 4.5*10^5;
ESig_X = zeros(Size_T,Size_t,XPts); % Initialize electric field due to polarization
%Ep_I = zeros(Size_T,Size_t,XPts);
...
for j = 1:Size_T
    ...
    parfor k = 1:XPts
        ...
        % mu, L, EIS, EID, gamma, E, probe
        [Time1,Y1] = ode23(@(t,y) timeEvol_2Lvl(t,y,Pos3+tAxis,ESig_X(j,:,k),mu,L,EIS,EID,gamma,E,probe),[TimeStart,TimeMax],y0,options);
        Y1 = Y1(:,1:NVar);
...
Pol3_X = reshape(Pol3_X,Size_T,Size_t,XPts);
ESig_X = 1i*mu*Pol3_X + ESig_X;
%Ep_R = mu*Y_All(:,:,3,:);
%Ep_I = mu*Y_All(:,:,2,:);

end

OutFile = strcat(Folder,'ESig_X',num2str(Index),'.dat');
dlmwrite(OutFile,ESig_X,'\t');
% size(ESig_X)

% Save population (rho_11)
Pop_X = Y_All(:,:,1,:);
OutFile = strcat(Folder,'Pop_X',num2str(Index),'.dat');
dlmwrite(OutFile,Pop_X,'\t');
